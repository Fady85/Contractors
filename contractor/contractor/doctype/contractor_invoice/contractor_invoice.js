...
frappe.ui.form.on("Contractor Invoice", {
        ...
        callback: (response) => {
          if (response.message) {
            ...
            frm.set_value(
              ...
              prevDoc.discount_percentage_amount
            );
            ...
            frm.set_value(
              ...
              prevDoc.insurance_percentage_amount
            );
  ...
  exchange_rate(frm) {
    frm.set_value(
      ...
      frm.doc.net_total * frm.doc.exchange_rate
    );
  },
  ...
  discount_total(frm) {
    frm.set_value(
      "net_total",
      frm.doc.grand_total - frm.doc.discount_total - frm.doc.insurance_total
    );
...
frappe.ui.form.on("Contractor Invoice Item", {
  ...
  subtotal(frm, cdt, cdn) {
    ...
    frappe.model.set_value(
      ...
      cdn,
      "grand_total",
      row.subtotal * (row.percentage / 100)
    );
    let total = 0;
    frm.doc.items.forEach((item) => {
      total += item.subtotal;
