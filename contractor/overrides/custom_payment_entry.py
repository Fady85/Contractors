...
class CustomPaymentEntry(PaymentEntry):
	...
	def before_submit(self):
		if(len(self.references)):
			for refrence in self.references:
				if(refrence.reference_doctype == "Employee Advance"):
					advanceDoc = frappe.get_doc("Employee Advance", refrence.reference_name);
					if(advanceDoc.advance_amount != self.paid_amount):
						frappe.throw("Please pay the complete amount of employee advance");
					else :
						frappe.db.set_value('Employee Advance', refrence.reference_name, 'status', 'Paid')
						frappe.db.set_value('Employee Advance', refrence.reference_name, 'paid_amount', advanceDoc.advance_amount)
